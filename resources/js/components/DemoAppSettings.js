...
const DemoAppSettings = () => {
    ...
    const saveSettings = (key) => {
        ...
        axios.post(`/api/setting/app.DemoApp.${key}`, formData)
            .then(() => {
                states[key] = 'saved';
                setStates({...states });
                setTimeout(function() {
    ...
    return ( <
        ...
        <
        label className = "block py-2 px-4"
        htmlFor = "memberWinPoints" > Member Points per - win < /label> <
        /div> <
        ...
        <
        label className = "block py-2 px-4"
        htmlFor = "teamWinPoints" > Team Points per - win < /label> <
        /div> <
        div className = "w-full lg:w-9/12" >
        <
        Input type = "text"
